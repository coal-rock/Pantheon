...
fn process_request(request: Request) -> Response {
    match request.action.as_str() {
        "heartbeat" => Response { status: "ok".to_string(), message: Some("Heartbeat received".to_string()) },
        _ => Response { status: "error".to_string(), message: Some("Unknown action".to_string()) }
    }
}
