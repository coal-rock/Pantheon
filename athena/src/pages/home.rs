...
pub fn home() -> Html {
    let github_url = "https://github.com/Dack985/Pantheon".to_string();
    let repo_api_url = "https://api.github.com/repos/Dack985/Pantheon".to_string();

    let star_count = use_state(|| None);

    {
        let star_count = star_count.clone();
        let repo_api_url = repo_api_url.clone(); // Ensure it's owned

        use_effect_with(repo_api_url.clone(), move |_| {
            spawn_local(async move {
                if let Ok(response) = Request::get(&repo_api_url).send().await {
                    if let Ok(repo_info) = response.json::<RepoInfo>().await {
                        star_count.set(Some(repo_info.stargazers_count));
                    }
                }
            });
            || () // Return a teardown function (no-op)
        });
    }
                ...
                >
                    ...
                    <div class="ml-2 flex items-center gap-1 text-sm md:flex">
                        ...
                        <span class="inline-block tabular-nums tracking-wider font-display font-medium text-white">
                           { star_count.map(|count| count.to_string()).unwrap_or_else(|| "...".to_string()) }
                        </span>
                    </div>
                </button>
            </div>
